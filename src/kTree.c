...
/*------------------------------------------------------------------
 ...
 TREP * compactCreateKTree(char * filein, char * fileout, uint subm,int _K1, int _K2, int levelcut){
 	...
 	ulong i,k,j,z,queuecont, conttmp;
...
//				fprintf(stderr,"row= %d, column=%d, edges from %d and from %d, (%2.2f\%)\n",fila,columna,fila*subm, columna*subm,(float)(fila*part+columna)*100/part/part);
		...
		uint * bits_BN;
		uint * bits_BT=(uint *) malloc(sizeof(uint)*nedges);
		for(i=0;i<nedges;i++)
			bits_BT[i]=0;
		bits = bits_BT;
	...
	for(i=0;i<trep->maxLevel1-1;i++){
		...
		for(k=0;k<queuecont;k++){
			for(j=0;j<K1;j++){
				...
				//printf("Case 1: node: %d (next: %d) level %d base %d %d\n",node,rank(rep->bt,node)*K1K1, i, trep->basex[trep->iniq]+j*div_level, div_level);
	...
	for(i=trep->maxLevel1-1;i<trep->maxLevel1;i++){
		...
		for(k=0;k<queuecont;k++){
			for(j=0;j<K1;j++){
				...
				//printf("Case 2: node: %d (next: %d) level %d, base %d %d\n",node,rep->cutBt + (rank(rep->bt,node)*K1K1-rep->cutBt)/factorAdjust,i, trep->basex[trep->iniq]+j*div_level, div_level);
				if(isBitSet(rep->bt,node)){
	...
	for(i=0;i<trep->maxLevel2-1;i++){
		...
		for(k=0;k<queuecont;k++){
			for(j=0;j<K2;j++){
				...
				//printf("Case 3: node: %d (next: %d) level %d, isbitset: %d base: %d %d\n",node,rep->cutBt + (rank(rep->bt,node-1)-cutPreRank)*K2K2,i,isBitSet(rep->bt,node), trep->basex[trep->iniq]+j*div_level, div_level);
				if(isBitSet(rep->bt,node)){
...
MREP * createRepresentation(NODE * root, uint numberOfNodes,ulong numberOfEdges){
	...
	bitRankW32Int *BT, *BN;
	/* rank() (bitrankw32int.c) hace "++i; ... br->data[i/W] ..." y por
	 * eso necesita hasta n/W+1 palabras reservadas (ese es justo el
	 * convenio que ya usa save(): "n/W+1"). ceil(n/W) = (n+W-1)/W se
	 * queda corta cuando n es múltiplo exacto de W: heap-buffer
	 * over-read confirmado con valgrind. Mismo bug que ya se documentó
	 * y arregló en Graph.hpp; se aplica aquí el mismo parche mínimo. */
	uint * bits_BT = (uint*)calloc(bits_BT_len/W + 1, sizeof(uint));
	uint * bits_BN = (uint*)calloc(bits_BN_len/W + 1, sizeof(uint));
	uint * bits_LI = (uint*)malloc(sizeof(uint)*((bits_LI_len+W-1)/W));
...
void saveTreeRep(TREP * trep, char * basename){
  ...
  int i;
  /* Antes leía de las globales hash/positionInTH, que dejan de ser válidas
   * si se liberan tras compressInformationLeaves (p.ej. para reconstruir
   * varios árboles en el mismo proceso sin fugas). trep->words ya es una
   * copia propia e independiente del vocabulario, así que usamos esa. */
  for (i=0;i<trep->zeroNode;i++)
  	fwrite(&(trep->words[i*trep->lenWords]),sizeof(char),trep->lenWords,fv);
					...
					for(ii=p1new;ii<=p2new;ii++)
						for(jj=q1new;jj<=q2new;jj++)
							...
							if(bitgetchar(&(trep->words[realvalue*trep->lenWords]),(jj%K2_2+(ii%K2_2)*K2_2))){
								...
								//fprintf(stderr,"--- link %d %d, baseold %d basenew %d\n",dp+i*K2_2+ii+trep->fila*trep->tamSubm,dq+j*K2_2+jj+trep->columna*trep->tamSubm, dq+j*K2_2,jj);
							}
						}
					}
				}
...
void recursiveDecompression(TREP * trep, MREP * rep, uint * tmp_in, uint * tmp_out, ulong * tmp_p, ulong * sizein, ulong * pointerL,uint dp, uint dq,uint x,int l);
...
void recursiveDecompression(TREP * trep, MREP * rep, uint * tmp_in, uint * tmp_out, ulong * tmp_p, ulong * sizein, ulong * pointerL,uint dp, uint dq,uint x,int l){
	...
	int i,j,leaf, realvalue;
	uint y, divlevel;
	if(l==trep->maxLevel1+trep->maxLevel2){	
		uint posInf =pointerL[l+1];
			realvalue = accessFT(rep->compressIL,posInf);
			pointerL[l+1]++;
			for(i=0;i<K2_2;i++)
				for(j=0;j<K2_2;j++){
					printf("Caso 5: x %d realvalue %d posInf %d\n",j+i*K2_2,realvalue, posInf);
					if(bitgetchar(&(trep->words[realvalue*trep->lenWords]),j+i*K2_2)){
						tmp_in[*tmp_p]=dp+i+trep->fila*trep->tamSubm;
						tmp_out[*tmp_p]=dq+j+trep->columna*trep->tamSubm;
						sizein[dp+i+trep->fila*trep->tamSubm]++;
						//if((dp+i+trep->fila*trep->tamSubm)==0)
						//printf("Anadiendo a %d, %d %d %d %d\n",dp+i+trep->fila*trep->tamSubm,dp,i,trep->fila,trep->tamSubm);
						(*tmp_p)++;
						//printf("--- link %d %d, baseold %d basenew %d, fila %d columna %d\n",dp+i+trep->fila*trep->tamSubm,dq+j+trep->columna*trep->tamSubm, dq,j,trep->fila,trep->columna);
						if(*tmp_p>100)
							exit(0);
					}
				}
	...
	if((l==trep->maxLevel1+trep->maxLevel2-1)){
		if(x>=rep->bn_len){
			printf("%d %d %d %d %d %d, %d %d\n",x,rep->bn_len,pointerL[l],l,dp, dq,trep->fila, trep->columna );
			exit(0);
		}

	}

	if((l==trep->maxLevel1+trep->maxLevel2-1)&& (isBitSet(rep->bn,x))){
		//y=rank(rep->bn,x)*K2K2;
		y = pointerL[l+1];
		//pointerL[l+1]+=K2K2;
		...
		//for(i=0;i<K2;i++){
 		//		for(j=0;j<K2;j++){
 		//			printf("Caso 4: %d, %d %d, baseold: %d basenew: %d divlevel: %d\n",x, y,rank(rep->bn,x)*K2*K2, dq,dq+j*K2_2, K2_2);
	 	//			recursiveDecompression(trep,rep,tmp_in, tmp_out, tmp_p,sizein,pointerL,dp + i*K2_2,dq+j*K2_2,y+K2*i+j,l+1);	 		
 		//	}
		//}
	...
	if((l>=trep->maxLevel1)&&(l<trep->maxLevel1+trep->maxLevel2-1)&& (isBitSet(rep->bt,x))){
		//y=rank(rep->bt,x)*K2*K2;
		y = pointerL[l+1];
		pointerL[l+1]+=K2K2;

		

		divlevel =	trep->div_level_table2[l+1-trep->maxLevel1];

		for(i=0;i<K2;i++){
 				for(j=0;j<K2;j++){
 					//if((trep->fila==0)&&(trep->columna==1))printf("Caso 3: %d %d %d, l %d baseold %d basenew: %d divlevel: %d\n",x,y+K2*i+j,rep->cutBt + (rank(rep->bt,x-1)-cutPreRank)*K2K2,l,dq,dq+j*divlevel, divlevel );
	 				recursiveDecompression(trep,rep,tmp_in, tmp_out, tmp_p,sizein,pointerL,dp + i*divlevel,dq+j*divlevel,y+K2*i+j,l+1);	 		
 			}
		}
	...
	if((l==trep->maxLevel1-1)&& (isBitSet(rep->bt,x))){
		//y=rank(rep->bt,x)*K1K1;
		y = pointerL[l+1];
		pointerL[l+1]+=K2K2;
		
		divlevel =	trep->div_level_table2[0];
		
		for(i=0;i<K2;i++){
 				for(j=0;j<K2;j++){
 					//if((trep->fila==0)&&(trep->columna==1))printf("Caso 2: %d %d %d, l: %d baseold %d basenew %d divlevel: %d\n",x, y+K2*i+j,(rank(rep->bt,x))*K1K1,l, dq,dq+j*divlevel, divlevel);
	 				recursiveDecompression(trep,rep,tmp_in, tmp_out, tmp_p,sizein,pointerL,dp + divlevel*i,dq+j*divlevel,y+K2*i+j,l+1);	 		
 			}
		}
 		...
 		if((x==-1)||((l<trep->maxLevel1-1)&&(isBitSet(rep->bt,x)))){
 			//y = (rank(rep->bt,x))*K1K1;
 			y = pointerL[l+1];
			pointerL[l+1]+=K1K1;
 			
 			divlevel =	trep->div_level_table1[l+1];
			for(i=0;i<K1;i++){
 				for(j=0;j<K1;j++){
 					//if((trep->fila==0)&&(trep->columna==1))printf("Caso 1: %d %d %d, l %d maxlevel1 %d baseold %d basenew %d divlevel: %d\n",x,y+K1*i+j,(rank(rep->bt,x))*K1K1,l, trep->maxLevel1,dq,dq+divlevel*j,divlevel );
	 				recursiveDecompression(trep,rep,tmp_in, tmp_out, tmp_p,sizein,pointerL,dp + divlevel*i,dq+divlevel*j,y+K1*i+j,l+1);	 		
 				}
 			}
...
int * compactFullDecompression(TREP * trep){
	ulong p = 0;
	uint * in = (uint *) malloc(sizeof(uint)*trep->numberOfEdges);
	uint * out = (uint *) malloc(sizeof(uint)*trep->numberOfEdges);
	ulong * sizein = (ulong *) malloc(sizeof(ulong)*(trep->numberOfNodes+1));
	int i;
	for (i=0;i<trep->numberOfNodes;i++)
		sizein[i]=0;
...
for(i=0;i<trep->part;i++){
		for(j=0;j<trep->part;j++){
			...
			ulong * pointerL = (ulong *) malloc(sizeof(ulong)*(trep->maxLevel1+trep->maxLevel2+2));
			pointerL[0]=0;
			pointerL[1]=K1K1;
			int t;
			int val=K1K1;

			for (t=2;t<trep->maxLevel1;t++){
				val=(rank(rep->bt,val-1)+1)*K1*K1;
				fprintf(stderr,"pointer[%d]=%d\n",t,val);
			}

			uint cutPreRank = rank(rep->bt,rep->lastBt1_len-1);

			for (t=trep->maxLevel1;t<=trep->maxLevel1+trep->maxLevel2-1;t++){
				val=rep->cutBt + (rank(rep->bt,val)-cutPreRank)*K2K2;// rep->cutBt + ((rank(rep->bt,pointerL[t-1]))*K1*K1-rep->cutBt)/factorAdjust;
				fprintf(stderr,"pointer[%d]=%d\n",t,val);
			}
		}
	}
...
//printf("%d %d %d %d %d\n",trep->numberOfNodes,trep->maxLevel1,trep->maxLevel2,trep->tamSubm,trep->part);
	for(i=0;i<trep->part;i++){
		for(j=0;j<trep->part;j++){
			//fprintf(stderr,"%d %d %d\n",i,j,trep->part);
			rep=trep->submatrices[i][j];
			...
			for (t=2;t<trep->maxLevel1;t++){
				pointerL[t]=(rank(rep->bt,pointerL[t-1]-1)+1)*K1*K1;
				//fprintf(stderr,"pointer[%d]=%d\n",t,pointerL[t]);
			}
			...
			for (t=trep->maxLevel1;t<=trep->maxLevel1+trep->maxLevel2-1;t++){
				pointerL[t]=rep->cutBt + (rank(rep->bt,pointerL[t-1]-1)-cutPreRank)*K2K2;// rep->cutBt + ((rank(rep->bt,pointerL[t-1]))*K1*K1-rep->cutBt)/factorAdjust;
				//fprintf(stderr,"pointer[%d]=%d\n",t,pointerL[t]);
			}

			pointerL[trep->maxLevel1+trep->maxLevel2-1]=0;
			pointerL[trep->maxLevel1+trep->maxLevel2]=1;
			//fprintf(stderr,"pointer[%d]=0\npointer[%d]=1\n",trep->maxLevel1+trep->maxLevel2-1,trep->maxLevel1+trep->maxLevel2);
			pointerL[trep->maxLevel1+trep->maxLevel2+1]=1;
			recursiveDecompression(trep,rep, in, out, &p, sizein, pointerL,0,0,-1,-1);

		}
	}

	
	int *lista = (int *) malloc(sizeof(int)*(trep->numberOfEdges+trep->numberOfNodes));
	...
	for (i=0;i<trep->numberOfNodes;i++){
		...
		uint *l  = compactTreeAdjacencyList(trep, i);
	    recovered = l[0];
		if(sizein[i]!=recovered){
			fprintf(stderr,"%d recovered: %d, listain: %d\n",i,recovered,sizein[i]);
			for(j=0;j<sizein[i];j++)
				fprintf(stderr,"%d ",in[j]);
			exit(0);
		}
	}
	...
	*/
	...
	sizein[trep->numberOfNodes] = suma;
	
	for(i=0;i<trep->numberOfEdges;i++){
		lista[sizein[in[i]]] = out[i]+1;
		sizein[in[i]]++;
	}

//	for(i=0;i<10;i++)
//		fprintf(stderr, "%d \t", lista[i] );
